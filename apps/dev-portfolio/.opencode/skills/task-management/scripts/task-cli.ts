...
function buildCompletedSeqs(subtasks: Subtask[]): Set<string> {
  const completedSeqs = new Set<string>();
  for (const subtask of subtasks) {
    if (subtask.status === 'completed') {
      completedSeqs.add(subtask.seq);
    }
  }
  return completedSeqs;
}

function indexSubtasks(subtasks: Subtask[]): Map<string, Subtask> {
  return new Map(subtasks.map((subtask) => [subtask.seq, subtask]));
}
...
function cmdNext(feature?: string): void {
  ...
  for (const f of features) {
    const subtasks = loadSubtasks(f);
    const completedSeqs = buildCompletedSeqs(subtasks);
...
function cmdDeps(feature: string, seq: string): void {
  ...
  const subtaskIndex = indexSubtasks(subtasks);
  const target = subtaskIndex.get(seq);
  ...
  const printDeps = (seqs: string[], indent: string = '  '): void => {
    for (let i = 0; i < seqs.length; i++) {
      const depSeq = seqs[i];
      const dep = subtaskIndex.get(depSeq);
...
function cmdComplete(feature: string, seq: string, summary: string): void {
  ...
  const subtasks = loadSubtasks(feature);
  const subtaskIndex = indexSubtasks(subtasks);
  const subtask = subtaskIndex.get(seq);
...
function cmdValidate(feature?: string): void {
  ...
  for (const f of features) {
    ...
    for (const s of subtasks) {
      const dependsOnSet = Array.isArray(s.depends_on) ? new Set(s.depends_on) : new Set<string>();
      ...
      if (dependsOnSet.has(s.seq)) {
        errors.push(`${s.seq}: task cannot depend on itself`);
      }

      // Check for missing dependencies
      for (const dep of dependsOnSet) {
        if (!seqs.has(dep)) {
      ...
      const checkCircular = (seq: string, path: string[], pathSet: Set<string>): boolean => {
        if (pathSet.has(seq)) {
        ...
        const task = subtaskIndex.get(seq);
        if (task) {
          const nextPath = [...path, seq];
          const nextSet = new Set(pathSet);
          nextSet.add(seq);
          for (const dep of task.depends_on) {
            if (checkCircular(dep, nextPath, nextSet)) return true;
          }
        }
        return false;
      };
      checkCircular(s.seq, [], new Set());
    }
